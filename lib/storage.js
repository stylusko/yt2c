...
export function ensureStorageDir() {
  [STORAGE_DIR, OVERLAYS_DIR, BACKGROUNDS_DIR, OUTPUTS_DIR, TEMP_DIR, FRAMES_DIR].forEach(dir => {
...
export function saveOverlay(jobId, base64Data) {
  ...
  // Detect format and remove data URL prefix
  const isWebp = base64Data.startsWith('data:image/webp');
  const ext = isWebp ? 'webp' : 'png';
  const data = base64Data.replace(/^data:image\/\w+;base64,/, '');
  const buffer = Buffer.from(data, 'base64');

  const overlayPath = path.join(OVERLAYS_DIR, `${jobId}.${ext}`);
...
/**
 * Gets frame file path
 * @param {string} filename - Frame filename (e.g. "abc123.png")
 * @returns {string} File path
 */
export function getFramePath(filename) {
  ensureStorageDir();
  return path.join(FRAMES_DIR, filename);
}

/**
 * Deletes a frame file
 * @param {string} filename - Frame filename
 */
export function deleteFrame(filename) {
  const fp = path.join(FRAMES_DIR, filename);
  if (fs.existsSync(fp)) {
    fs.unlinkSync(fp);
  }
}
