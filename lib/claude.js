...
## 배민전용 제목 길이 규칙
- 배민전용 표지는 제목이 매우 크게 렌더링되므로 일반 카드뉴스보다 훨씬 짧게 작성
- 첫 번째 커버 카드 title: 전체 12자 이내, 줄당 8자 이내, 최대 2줄
- 나머지 카드 title: 전체 24자 이내, 줄당 12자 이내, 최대 2줄
- 긴 설명, 조건, 숫자 맥락은 title에 넣지 말고 body로 분리
- title은 핵심 명사와 짧은 동사 위주로 작성` : '';
...
${transcript}`;
  ...
  if (isBaeminOnly) {
    clips = clips.map((clip, idx) => {
      const isCover = !!clip.is_cover || idx === 0;
      return {
        ...clip,
        title: fitShortHeadline(clip.title, {
          maxChars: isCover ? 12 : 24,
          maxLineChars: isCover ? 8 : 12,
          maxLines: 2,
        }),
      };
    });
...
- sourceImageIndex는 null.`;
  ...
  const sourceInstruction = sourceLabel
    ...
    : '';
  const baeminCopyInstruction = isBaeminOnly ? `\n\n## 배민전용 카피 길이 규칙\n- 배민전용 레이아웃은 표지 제목이 매우 크게 렌더링되므로 headline/subtext를 짧게 작성.\n- 첫 번째 cover headline: 전체 12자 이내, 줄당 8자 이내, 최대 2줄.\n- 첫 번째 cover subtext: 전체 12자 이내, 가능하면 1줄.\n- content headline: 전체 24자 이내, 줄당 12자 이내.\n- 긴 조건, 숫자, 설명은 body에 넣고 headline/subtext에는 핵심어만 남길 것.` : '';
...
JSON만 반환.`;
  ...
  parsed.cards = parsed.cards.map((c, i) => {
    ...
    const headline = isBaeminOnly
      ? fitShortHeadline(rawHeadline, { maxChars: i === 0 ? 12 : 24, maxLineChars: i === 0 ? 8 : 12, maxLines: 2 })
      : rawHeadline.slice(0, 60);
    const subtext = isBaeminOnly
      ? fitShortHeadline(rawSubtext, { maxChars: i === 0 ? 12 : 20, maxLineChars: i === 0 ? 8 : 10, maxLines: i === 0 ? 2 : 1 })
