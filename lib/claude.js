...
function stripClaudeJsonFence(text) {
  const trimmed = String(text || '').trim();
  const fenced = trimmed.match(/^```(?:json)?\s*([\s\S]*?)\s*```$/i);
  return (fenced ? fenced[1] : trimmed).trim();
}

function parseJsonCandidate(responseText) {
  const text = stripClaudeJsonFence(responseText);
  try {
...
export function parseArticleCardsResponse(responseText, fallbackTitle = '') {
  let parsed;
  try {
    parsed = parseJsonCandidate(responseText);
  } catch (e) {
    throw new Error('Claude 응답에서 JSON을 파싱할 수 없습니다.');
  }
...
JSON만 반환.`;
  ...
  const parsed = parseArticleCardsResponse(responseText, article.title || '');
