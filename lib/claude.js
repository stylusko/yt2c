...
- 마지막 카드는 요약/액션/출처 안내
- 중간 카드들은 기사의 서사 흐름을 따라가되, 카드 간 논리적 연결 유지

## 문구 작성 규칙
- headline: 20자 이내. 두 줄 쓸 때는 \\n으로 구분
- subtext: 40자 이내의 보조 설명 (선택적, 필요 없으면 빈 문자열)
- 자극적 표현/과장/허구 금지
- 명사형 종결 기본, 가끔 ~요/~다 섞어 자연스럽게
...
export async function divideArticleToCards(article, options = {}) {
  ...
  const cardCountHint = options.cardCount === 'auto' || !options.cardCount
    ? '본문 길이에 맞춰 자동 결정 (5~12장 권장)'
    : `정확히 ${options.cardCount}장`;
...
위 기사를 카드뉴스로 분할해주세요. JSON만 반환.`;
  ...
  // reuse 카드의 sourceImageIndex가 유효한지 체크, 아니면 generate로 강등
  parsed.cards.forEach(c => {
    if (c.imageStrategy === 'reuse') {
      if (c.sourceImageIndex == null || !article.images || c.sourceImageIndex >= article.images.length) {
        c.imageStrategy = c.imagePrompt ? 'generate' : 'none';
      }
    }
  });
