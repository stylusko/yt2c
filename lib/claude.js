...
const BAEMIN_VIDEO_POST_BODY_LAYOUT_IDS = ['bm-video-post-title-top', 'bm-video-post-title-bottom'];
...
function baeminRecommendationCoverLayoutId(card, aspectRatio, layoutPreference = null) {
  ...
  if (card?.hasVisual) return vertical ? 'bm-cover-reels' : 'bm-cover-square';
  return vertical ? 'bm-cover-feed' : 'bm-cover-text-square';
}

function isBaeminVideoPostRecommendationCandidate(card, index, aspectRatio, sourceType) {
...
export function baeminAllowedLayoutIdsForRecommendation(card, index, aspectRatio, sourceType, layoutPreference = null) {
  if (isBaeminVideoPostRecommendationCandidate(card, index, aspectRatio, sourceType)) {
    return BAEMIN_VIDEO_POST_BODY_LAYOUT_IDS;
  }
  if (sourceType !== 'article' || !layoutPreference) return BAEMIN_LAYOUT_IDS;
  const preference = normalizeBaeminArticleLayoutPreference(layoutPreference);
  if (index === 0) return [baeminRecommendationCoverLayoutId(card, aspectRatio, preference)];
  if (preference.body === 'text') return BAEMIN_ARTICLE_TEXT_LAYOUT_IDS;
  if (!card?.hasVisual) return BAEMIN_ARTICLE_TEXT_LAYOUT_IDS;
...
// 카드 배경 이미지는 photo_top 레이아웃(이미지 상단, 하단에 텍스트 오버레이) 위에 깔린다.
// 따라서 피사체를 상·중앙에 두고 하단을 비워 텍스트 가독성을 확보하는 구도가 필요.
// 모든 프리셋에 공통 주입해 카드 간 구도 일관성을 보장한다.
const ARTICLE_COMPOSITION_GUIDE =
  'single clear focal subject placed in the upper-center of the frame, generous calm negative space in the lower third reserved for a text caption, uncluttered balanced composition, nothing important near the bottom edge, subject not cropped at the edges';

// 텍스트/로고 렌더링을 강하게 억제하는 네거티브 (한글 깨짐·워터마크 방지).
const ARTICLE_IMAGE_NEGATIVES =
  'absolutely no text, no letters, no words, no numbers, no typography, no captions, no labels, no logos, no watermarks, no UI elements';

/**
 * 카드 AI 이미지의 최종 프롬프트를 조립한다. (생성/재생성 두 경로 공통)
 * gpt-image-2는 자연어 지시를 잘 따르므로 "주제 → 스타일 → 구도 → 네거티브" 순의
 * 구조화된 문장으로 구성한다. 주제를 맨 앞에 둬 스타일 수식어가 주제를 압도하지 않게 한다.
 * @param {object} params
 * @param {string} params.subject - Claude가 작성한 장면 묘사(스타일 제외)
 * @param {string} [params.presetId]
 * @returns {string}
 */
export function composeArticleImagePrompt({ subject, presetId } = {}) {
  const preset = getArticleStylePreset(presetId);
  const cleanSubject = String(subject || '').trim().replace(/[,\s]+$/, '');
  return [
    `Subject: ${cleanSubject}.`,
    `Style: ${preset.stylePrefix}.`,
    `Composition: ${ARTICLE_COMPOSITION_GUIDE}.`,
    ARTICLE_IMAGE_NEGATIVES + '.',
  ].join(' ');
}
...
- **영어로, 12~25단어**의 구체적 장면 묘사. 너무 짧으면(예: "hands taking notes") 밋밋한 이미지가 나오니 금지.
- **그 카드 본문의 핵심 의미를 직접 시각화**할 것. 추상적·일반적 장면이 아니라 본문이 말하는 구체적 상황·사물·행동을 담는다.
- 다음 4요소를 반드시 포함: ① 주된 피사체(사물/손/공간) ② 배경·장소 ③ 핵심 소품 1~2개 ④ 조명·분위기
- 사람 얼굴/실존 인물/유명인/브랜드 로고 금지 (손·실루엣·뒷모습·사물·공간으로 대체)
- 스타일(사진풍/3D/일러스트 등)·화질·구도·"no text" 같은 설명은 **넣지 말 것** — 시스템이 자동 주입한다. 순수 "무엇을 그릴지(장면)"만 작성.
- 좋은 예: "an open notebook on a wooden cafe table with a steaming coffee cup and a pen resting on a handwritten to-do list, soft warm morning light from a side window"
- 나쁜 예: "hands taking notes" (너무 짧고 일반적), "business concept illustration" (추상적·내용과 무관)
...
type은 "cover" (첫 카드 1장) 또는 "content" (나머지 전부) 둘 중 하나만 사용:
{
  ...
  "cards": [
    ...
    },
    {
      ...
      "imagePrompt": "an open notebook on a wooden desk with a steaming coffee cup and a pen on a handwritten checklist, warm morning light from a side window"
    },
    {
      "type": "content",
      "headline": "",
...
function normalizeBaeminRecommendationPlan(parsed, cards = [], options = {}) {
  ...
  const layoutPreference = sourceType === 'article' && options.layoutPreference
    ? normalizeBaeminArticleLayoutPreference(options.layoutPreference)
    : null;
  ...
  source.forEach(item => {
    ...
    const allowedIds = baeminAllowedLayoutIdsForRecommendation(cards[cardIndex], cardIndex, aspectRatio, sourceType, layoutPreference);
    if (!allowedIds.includes(layoutId)) return;
...
export async function recommendBaeminLayouts(cards, options = {}) {
  ...
  const layoutPreference = sourceType === 'article' && options.layoutPreference
    ? normalizeBaeminArticleLayoutPreference(options.layoutPreference)
    : null;
  const allowedLayoutGuide = safeCards.map((card, index) => ({
    cardIndex: index,
    allowedLayoutIds: baeminAllowedLayoutIdsForRecommendation(card, index, aspectRatio, sourceType, layoutPreference),
  }));
  const client = new Anthropic({ apiKey });
...
${layoutPreference ? `## 사용자 선택 레이아웃 정책
- 표지: ${layoutPreference.cover === 'photo' ? '사진 표지' : '텍스트 표지'}
- 본문: ${layoutPreference.body === 'photo' ? `사진 배경(${layoutPreference.photoTreatment === 'frame' ? '텍스트 박스' : '그라데이션'})` : '텍스트 위주'}
- 각 카드의 allowedLayoutIds 안에서만 고릅니다.
` : ''}
...
layoutPreference: ${layoutPreference ? JSON.stringify(layoutPreference) : 'none'}
allowedLayoutGuide:
${JSON.stringify(allowedLayoutGuide, null, 2)}
...
각 카드에 가장 적절한 배민 전용 layoutId를 선택하세요.`;
  ...
  return {
    recommendations: normalizeBaeminRecommendationPlan(parsed, safeCards, { aspectRatio, sourceType, layoutPreference }),
    model: CLAUDE_MODEL,
  };
}
...
export async function divideArticleToCards(article, options = {}) {
  ...
  const isBaeminOnly = options.brandGuide === 'baemin-only';
  const baeminLayoutPreference = isBaeminOnly
    ? normalizeBaeminArticleLayoutPreference(options.layoutPreference)
    : null;
  ...
  // 모드별 시스템 프롬프트 부록
  const modeInstruction = imageMode === 'manual'
    ? `## 이미지 모드: 직접 삽입 (본문 이미지 우선 + 부족분 공백)
...
- 카드 수가 이미지 수를 초과하면:
  ...
  2) 남은 카드는 "none"으로 지정. imagePrompt는 비워둠.
...
- "generate"는 사용 금지.
${availableImages === 0 ? '- 원본 이미지가 없으므로 모든 카드를 "none"으로 해야 합니다.' : ''}`
    : imageMode === 'reuse'
...
- sourceImageIndex는 null.`;
  ...
  const baeminLayoutInstruction = isBaeminOnly && baeminLayoutPreference ? `\n\n## 배민전용 레이아웃 선택값
- 표지: ${baeminLayoutPreference.cover === 'photo' ? '사진 표지' : '텍스트 표지'}
- 본문: ${baeminLayoutPreference.body === 'photo' ? `사진 배경(${baeminLayoutPreference.photoTreatment === 'frame' ? '텍스트 박스' : '그라데이션'})` : '텍스트 위주'}
- 이 레이아웃 선택값은 이미지 모드보다 우선합니다.
- 텍스트 표지 카드는 imageStrategy를 "none"으로 둡니다.
- 텍스트 위주 본문 카드는 모두 imageStrategy를 "none"으로 둡니다.
- 사진 표지 또는 사진 배경 본문처럼 이미지가 필요한 카드는 ${imageMode === 'manual' ? '"reuse" 또는 "none"을 사용합니다. 부족분은 사용자가 직접 삽입할 수 있도록 "none"으로 둡니다.' : '"reuse" 또는 "generate"를 사용하고, "none"을 쓰지 않습니다.'}` : '';

  const systemPrompt = ARTICLE_DIVIDE_SYSTEM + `\n\n${modeInstruction}${sourceInstruction}${baeminCopyInstruction}${baeminLayoutInstruction}\n\n## 이번 작업\n- 카드 수: ${cardCountHint}\n- 톤: ${ARTICLE_TONE_GUIDE[tone]}\n`;
...
JSON만 반환.`;
    ...
    if (availableImages === 0) {
      ...
      parsed.cards.forEach(c => {
        ...
        if (imageMode !== 'manual' && !c.imagePrompt) {
          ...
          c.imagePrompt = `a clear editorial scene that literally depicts the concept of "${hint}", a single focal real-world subject in a realistic setting with natural lighting`;
        }
      });
    } else {
  ...
  if (isBaeminOnly && baeminLayoutPreference) {
    const usedIndices = new Set();
    const nextReusableIndex = () => {
      for (let i = 0; i < availableImages; i++) {
        if (!usedIndices.has(i)) return i;
      }
      return null;
    };
    const ensurePrompt = (c) => {
      if (c.imagePrompt) return;
      const hint = (c.body || c.headline || c.subtext || 'editorial scene').slice(0, 80);
      c.imagePrompt = `a clean editorial photo that literally depicts the concept of "${hint}", a single focal real-world subject in a realistic setting with natural lighting`;
    };

    parsed.cards.forEach((c, index) => {
      const needsVisual = index === 0
        ? baeminLayoutPreference.cover === 'photo'
        : baeminLayoutPreference.body === 'photo';

      if (!needsVisual) {
        c.imageStrategy = 'none';
        c.sourceImageIndex = null;
        c.imagePrompt = '';
        return;
      }
      ...
      if (imageMode === 'manual') {
        const currentIndex = typeof c.sourceImageIndex === 'number' ? c.sourceImageIndex : null;
        const reuseIndex = currentIndex != null && currentIndex >= 0 && currentIndex < availableImages && !usedIndices.has(currentIndex)
          ? currentIndex
          : nextReusableIndex();

        if (reuseIndex != null) {
          usedIndices.add(reuseIndex);
          c.imageStrategy = 'reuse';
          c.sourceImageIndex = reuseIndex;
          c.imagePrompt = '';
        } else {
          c.imageStrategy = 'none';
          c.sourceImageIndex = null;
      ...
      if (imageMode === 'generate' || availableImages === 0) {
        c.imageStrategy = 'generate';
        c.sourceImageIndex = null;
        ensurePrompt(c);
        return;
  ...
  return {
    ...
    tone,
    imageMode,
    layoutPreference: baeminLayoutPreference,
  };
}
