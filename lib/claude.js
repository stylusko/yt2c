 ...
 */
export async function analyzeHighlights(transcript, videoTitle, duration, tone = 'hooking', textMode = 'title') {
...
- JSON 응답의 각 카드 객체에 "body": "..." 필드를 반드시 포함` : '';
  const systemPrompt = SYSTEM_PROMPT_BASE + '\n\n' + TONE_PROMPTS[validTone] + bodyAddon;
...
function articleCharsPerLine({ hasHeadline = false } = {}) {
  // Generated article cards use bodySize 40 without a headline and 36 with a headline.
  // Keep this slightly conservative so the final canvas stays at 4 lines or fewer.
  return hasHeadline ? 29 : 27;
}
...
위 기사를 카드뉴스로 분할해주세요. JSON만 반환.`;
  ...
  const parsed = extractAnthropicToolInput(message, ARTICLE_CARDS_TOOL_NAME)
  ...
  parsed.cards = parsed.cards.map((c, i) => {
    const headline = normalizeArticleSourceNames(c.headline || '', sourceUrl).slice(0, 60);
    const subtext = normalizeArticleSourceNames(c.subtext || '', sourceUrl).slice(0, 100);
