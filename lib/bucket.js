...
import { getCardCacheData } from './card-cache-hash.js';
...
export async function uploadBuffer(key, buffer, contentType) {
  ...
  try {
    ...
    console.log(`[bucket] uploaded: ${key} (${(buffer.length / 1024).toFixed(0)}KB)`);
    return key;
  } catch (err) {
...
export async function getObject(key) {
  ...
  if (!s3) return null;
  try {
    return await s3.send(new GetObjectCommand({ Bucket: BUCKET_NAME, Key: key }));
  } catch (err) {
    console.error('[bucket] get object failed:', err.message);
    return null;
  }
}
...
/**
 * prefix 아래 모든 객체 키 목록 조회 (페이지네이션 포함)
 * @param {string} prefix
...
export async function listKeys(prefix) {
  ...
  do {
    const cmd = new ListObjectsV2Command({ Bucket: BUCKET_NAME, Prefix: prefix, ContinuationToken: continuationToken });
...
export function generateCardHash(card, globalConfig) {
  const data = getCardCacheData(card, globalConfig, ARTICLE_IMAGE_RENDER_VERSION);
