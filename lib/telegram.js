...
function formatCardKind(cards) {
  if (!Array.isArray(cards) || cards.length === 0) return '-';
  const kinds = new Set(cards.map((card) => {
    const ext = String(card?.ext || '').toLowerCase();
    if (ext === 'mp4') return '영상';
    if (ext === 'jpg' || ext === 'jpeg' || ext === 'png' || ext === 'webp') return '이미지';
    return ext || '알 수 없음';
  }));
  if (kinds.size === 1) return [...kinds][0];
  const preferred = ['이미지', '영상'];
  return [
    ...preferred.filter((kind) => kinds.has(kind)),
    ...[...kinds].filter((kind) => !preferred.includes(kind)),
  ].join('+');
}
...
export async function notifyGroupComplete(jobId, completedCount, failedCount, durationSec, details = {}) {
  ...
  const formatName = formatCardKind(completedCards);
  // 인증 경로 표시
