...
// --- Proxy support (reuse from worker.js env) ---
let proxyList = [];
let proxyIndex = 0;
...
async function runYtDlp(args, opts = {}) {
  ...
  const baseArgs = [...args];
  if (fs.existsSync(COOKIE_PATH)) baseArgs.unshift('--cookies', COOKIE_PATH);

  // 1차: 프록시 없이 시도
  try {
    return await execFileAsync('yt-dlp', baseArgs, { timeout: opts.timeout || 60000 });
  } catch (firstErr) {
    if (proxyList.length === 0) throw firstErr;
    console.warn('[subtitle] 1차 실패, 프록시로 재시도...');
  }

  // 2차: 프록시로 재시도 (최대 3개)
  const maxAttempts = Math.min(proxyList.length, 3);
  let lastErr;
  for (let i = 0; i < maxAttempts; i++) {
    const proxy = getNextProxy();
    try {
      return await execFileAsync('yt-dlp', ['--proxy', proxy, ...baseArgs], { timeout: opts.timeout || 60000 });
    } catch (err) {
      lastErr = err;
      console.warn(`[subtitle] 프록시 ${i + 1}/${maxAttempts} 실패`);
    }
  }
  throw lastErr;
}
...
export async function getVideoInfo(url) {
  ...
  const { stdout } = await runYtDlp(['--dump-json', '--no-download', '--no-playlist', canonicalUrl], { timeout: 30000 });
...
export async function extractSubtitles(url, videoId, onStatus) {
  ...
  const ytSub = await fetchYouTubeSubtitles(canonicalUrl, videoId);
  ...
  const whisperResult = await whisperSTT(canonicalUrl, videoId, onStatus);
  return {
    ...whisperResult,
