...
export function getCardCacheData(card, globalConfig = {}, articleImageRenderVersion = 0) {
  ...
  return {
    ...
    tls: card.titleLetterSpacing ?? 0, tlh: card.titleLineHeight ?? 1.4, tx: card.titleX ?? 0, ty: card.titleY ?? 0,
    ss: card.subtitleSize || 48, sc: card.subtitleColor || '#aaa', sf: card.subtitleFont || '',
    sls: card.subtitleLetterSpacing ?? 0, slh: card.subtitleLineHeight ?? 1.4, sx: card.subtitleX ?? 0, sy: card.subtitleY ?? 0,
    bs: card.bodySize || 40, bc: card.bodyColor || '#d2d2d2', bf: card.bodyFont || '',
    bls: card.bodyLetterSpacing ?? 0, blh: card.bodyLineHeight ?? 1.4, bx: card.bodyX ?? 0, by: card.bodyY ?? 0,
