...
// Write YouTube cookies from env var to file (once)
// YT_COOKIES_B64: base64-encoded cookies.txt content
// YT_COOKIES: plain text fallback
...
function ensureCookieFile() {
  ...
  if (b64) {
    ...
    console.log('YouTube cookies file written (base64)');
  } else if (plain) {
    fs.writeFileSync(COOKIE_PATH, plain, 'utf-8');
    console.log('YouTube cookies file written (plain)');
  }
  cookieReady = true;
}
...
async function downloadYouTubeSegment(url, startSec, endSec, outputPath) {
  ...
  try {
    const listArgs = ['-F', '--js-runtimes', 'node', '--no-playlist'];
    if (fs.existsSync(COOKIE_PATH)) listArgs.push('--cookies', COOKIE_PATH);
    listArgs.push(url);
    const { stdout: fmtList } = await execFileAsync('yt-dlp', listArgs, { timeout: 30000 });
    console.log(`  Available formats:\n${fmtList}`);
  } catch (e) { console.log(`  Format listing failed: ${e.message}`); }
  ...
  await execFileAsync('yt-dlp', args, { timeout: 120000 }); // 2 minute timeout
}
