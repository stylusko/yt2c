...
function buildFilterChain(vw, vh, outW, outH, videoPos, videoScale, videoFill, layout, photoRatio, videoBrightness) {
  ...
  if (videoFill === 'split' && layout !== 'text_overlay' && layout !== 'none') {
    ...
    // Create: scale → crop to video area → brightness → pad to full output with black → format → overlay
    return `[0:v]scale=${scaleW}:${scaleH},crop=${outW}:${videoH}:${offX}:${offY}${eqFilter},pad=${outW}:${outH}:0:${videoY}:black,format=yuv420p[scaled];[1:v]format=yuva420p[ov];[scaled][ov]overlay=0:0`;
  }
  ...
  return `[0:v]scale=${scaleW}:${scaleH},crop=${outW}:${outH}:${offX}:${offY}${eqFilter},format=yuv420p[scaled];[1:v]format=yuva420p[ov];[scaled][ov]overlay=0:0`;
}
...
export async function processCard({
  ...
  try {
    ...
    // Low resolution detection + optional cookie expiry suspicion alert
    if (width <= 720) {
