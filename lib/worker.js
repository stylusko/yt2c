...
async function downloadYouTubeSegment(url, startSec, endSec, outputPath) {
  ...
  // 프록시 로테이션: 각 시도마다 다른 프록시 사용
  // 최대 시도 횟수 = 프록시 수(최소 1) × 2 (각 프록시당 2번 시도), 상한 6회
  const maxAttempts = Math.min(Math.max(proxyList.length, 1) * 2, 6);
  const retryDelayMs = 5000; // 프록시 전환 시 5초 대기
  let lastErr;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (proxy) {
      ...
      console.log(`  yt-dlp: attempt ${attempt}/${maxAttempts}, proxy: ${proxy.replace(/\/\/.*@/, '//***@')}`);
    }
    ...
    try {
      ...
      if (proxy) triedProxies.add(proxy);

      // 프록시 관련 오류거나 봇 감지 → 다음 프록시로 전환
      if ((isBot || is403 || isNetwork || isProxyErr) && attempt < maxAttempts) {
        const reason = isBot ? 'bot감지' : is403 ? '403' : isProxyErr ? '프록시오류' : 'network';
        console.warn(`  yt-dlp: attempt ${attempt} 실패 (${reason}), 다음 프록시로 전환...`);
