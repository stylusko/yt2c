...
function buildFilterChain(vw, vh, outW, outH, videoPos, videoScale, videoFill, layout, photoRatio, videoBrightness) {
  ...
  // 프론트엔드 오프셋 공식과 일치하도록 위치 비율(0-100) 계산
  // Frontend: offX = scaledW * videoX / 400 + (scaledW - width) / 2
  // → posX = clamp(50 + scaleD * rawVal / (4 * (scaleD - targetD)), 0, 100)
  function computePos(rawVal, scaleD, targetD) {
    const diff = scaleD - targetD;
    if (Math.abs(diff) < 1) return 50; // 정확히 맞으면 위치 무관
    return Math.max(0, Math.min(100, 50 + scaleD * (rawVal ?? 0) / (4 * diff)));
  }

  // Unified scale → pad → crop pipeline (handles zoom < 1 and zoom >= 1)
  function buildScalePadCrop(scaleW, scaleH, targetW, targetH) {
    const posX = computePos(rawX, scaleW, targetW);
    const posY = computePos(rawY, scaleH, targetH);
  ...
  if (videoFill === 'split' && layout !== 'text_overlay' && layout !== 'none') {
    ...
    const spc = buildScalePadCrop(scaleW, scaleH, outW, videoH);
  ...
  const spc = buildScalePadCrop(scaleW, scaleH, outW, outH);
...
async function downloadYouTubeSegment(url, startSec, endSec, outputPath) {
  ...
  try {
    ...
    if ((isBot || is403 || isNetwork) && proxyList.length > 0) {
      const reason = isBot ? 'bot감지' : is403 ? '403' : 'network';
  ...
  for (let attempt = 1; attempt <= maxProxyAttempts; attempt++) {
    ...
    try {
      ...
      if ((isBot || is403 || isNetwork || isProxyErr) && attempt < maxProxyAttempts) {
        const reason = isBot ? 'bot감지' : is403 ? '403' : isProxyErr ? '프록시오류' : 'network';
