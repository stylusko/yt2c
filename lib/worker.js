...
function buildFilterChain(vw, vh, outW, outH, videoPos, videoScale, videoFill, layout, photoRatio, videoBrightness, fitMode) {
  ...
  if (videoFill === 'split' && layout !== 'text_overlay' && layout !== 'none') {
    ...
    // Cover 모드: even() 반올림으로 검정 테두리 방지
    if (!splitContain && zoom >= 1) {
      if (scaleW < outW) scaleW = even(outW);
      if (scaleH < videoH) scaleH = even(videoH);
    }
  ...
  // Cover 모드: even() 반올림으로 1-2px 부족해져 검정 테두리 방지
  if (!useContain && zoom >= 1) {
    if (scaleW < outW) scaleW = even(outW);
    if (scaleH < outH) scaleH = even(outH);
  }
