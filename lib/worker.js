...
function buildFilterChain(vw, vh, outW, outH, videoPos, videoScale, videoFill, layout, photoRatio, videoBrightness, fitMode) {
  ...
  function buildVideoFilter(scaleW, scaleH, targetW, targetH) {
    ...
    // format=yuv420p를 먼저 적용해 PNG/WebP 알파 채널로 인한 녹색 화면 방지
    if (scaleW >= targetW && scaleH >= targetH) {
      ...
      return `[0:v]format=yuv420p,scale=${scaleW}:${scaleH}${eqFilter},crop=${targetW}:${targetH}:${cropX}:${cropY}`;
    }
    return `[0:v]format=yuv420p,scale=${scaleW}:${scaleH}${eqFilter}[sv];color=black:${targetW}x${targetH},format=yuv420p[bg];[bg][sv]overlay=${posX}:${posY}:shortest=1`;
  }
