...
async function getImageDimensions(imagePath) {
  // Check file exists and has valid size before running ffprobe
  try {
    const stat = fs.statSync(imagePath);
    if (stat.size === 0) {
      throw new Error(`배경 이미지 파일이 비어있습니다 (0 bytes): ${imagePath}`);
    }
    if (stat.size < 100) {
      console.warn(`[getImageDimensions] WARNING: file is only ${stat.size} bytes: ${imagePath}`);
    }
  } catch (statErr) {
    if (statErr.code === 'ENOENT') throw new Error(`배경 이미지 파일을 찾을 수 없습니다: ${imagePath}`);
    throw statErr;
  }
  try {
    const { stdout } = await execFileAsync('ffprobe', [
    ...
    const stat = fs.statSync(imagePath);
    // Read first 16 bytes to check file signature (magic bytes)
    const fd = fs.openSync(imagePath, 'r');
    const headerBuf = Buffer.alloc(16);
    fs.readSync(fd, headerBuf, 0, 16, 0);
    fs.closeSync(fd);
    const hex = headerBuf.toString('hex').substring(0, 32);
    throw new Error(`배경 이미지를 읽을 수 없습니다 (${stat.size} bytes, header: ${hex}): ${err.message}`);
  }
}
