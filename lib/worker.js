...
function buildFilterChain(vw, vh, outW, outH, videoPos, videoScale, videoFill, layout, photoRatio, videoBrightness, fitMode) {
  ...
  function buildVideoFilter(scaleW, scaleH, targetW, targetH) {
    ...
    // format=yuv420p는 scale 후에 적용 — 1x1 같은 작은 입력이 YUV 4:2:0 서브샘플링 실패로
    // U=V=0이 되어 lime green(#00C800 부근)으로 출력되는 문제 방지
    if (scaleW >= targetW && scaleH >= targetH) {
      ...
      return `[0:v]scale=${scaleW}:${scaleH}${eqFilter},crop=${targetW}:${targetH}:${cropX}:${cropY},format=yuv420p`;
    }
    return `[0:v]scale=${scaleW}:${scaleH}${eqFilter},format=yuv420p[sv];color=black:${targetW}x${targetH},format=yuv420p[bg];[bg][sv]overlay=${posX}:${posY}:shortest=1`;
  }
