...
const BAEMIN_VIDEO_POST_SPECS = {
  [BAEMIN_VIDEO_POST_TITLE_TOP_LAYOUT]: { videoTop: 320, videoHeight: BAEMIN_VIDEO_POST_VIDEO_HEIGHT },
  [BAEMIN_VIDEO_POST_TITLE_BOTTOM_LAYOUT]: { videoTop: 200, videoHeight: BAEMIN_VIDEO_POST_VIDEO_HEIGHT },
};

function getBaeminVideoPostArea(layout, outW, outH) {
  const spec = BAEMIN_VIDEO_POST_SPECS[layout];
  if (!spec) return null;
  const sy = outH / BAEMIN_VIDEO_POST_DESIGN_HEIGHT;
  return {
 ...
 * @param {boolean} clampCoverZoom - optionally keep cover-fit sources at least 101% scale
 */
function buildFilterChain(vw, vh, outW, outH, videoPos, videoScale, videoFill, layout, photoRatio, videoBrightness, fitMode, clampCoverZoom = true) {
  ...
  const coverZoom = clampCoverZoom ? Math.max(zoom, 1.01) : zoom;
  const aspect = vw / vh;
  ...
  const baeminVideoPostArea = getBaeminVideoPostArea(layout, outW, outH);
  if (videoFill === 'split' && baeminVideoPostArea) {
    const videoH = baeminVideoPostArea.videoHeight;
    const videoY = baeminVideoPostArea.videoTop;
...
async function downloadYouTubeSegment(url, startSec, endSec, outputPath, onStatusMessage) {
  ...
  if (!canonicalUrl) throw new Error('Invalid YouTube URL');
...
export { ensureCookieFile, downloadYouTubeSegment, getVideoDimensions, buildFilterChain };
