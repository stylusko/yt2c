...
async function downloadYouTubeSegment(url, startSec, endSec, outputPath) {
  ...
  try {
    ...
    const isGeoRestricted = stderr.includes('not made this video available in your country') || stderr.includes('is not available');

    if ((isBot || is403 || isNetwork || isCorrupt || isGeoRestricted) && proxyList.length > 0) {
      const reason = isBot ? 'bot감지' : is403 ? '403' : isGeoRestricted ? '지역제한' : isCorrupt ? '파일손상' : 'network';
  ...
  for (let attempt = 1; attempt <= maxProxyAttempts; attempt++) {
    ...
    try {
      ...
      if ((isBot || is403 || isNetwork || isProxyErr || isCorrupt || isGeoRestricted) && attempt < maxProxyAttempts) {
        const reason = isBot ? 'bot감지' : is403 ? '403' : isGeoRestricted ? '지역제한' : isCorrupt ? '파일손상' : isProxyErr ? '프록시오류' : 'network';
