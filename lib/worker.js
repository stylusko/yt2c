...
async function downloadYouTubeSegment(url, startSec, endSec, outputPath) {
  ...
  try {
    ...
    const isGeoBlock = stderr.includes('not made this video available in your country') || stderr.includes('geo') || stderr.includes('not available in your country');

    if ((isBot || is403 || isNetwork || isCorrupt || isGeoBlock) && proxyList.length > 0) {
      const reason = isGeoBlock ? '지역제한' : isBot ? 'bot감지' : is403 ? '403' : isCorrupt ? '파일손상' : 'network';
