...
function VideoPreview({ videoId, start, end, width, height, videoX, videoY, videoScale, videoBrightness, muted: mutedProp, volume: volumeProp = 80 }) {
  ...
  const offX = scaledW * ((videoX ?? 0) + 200) / 400 - width / 2;
  const offY = scaledH * ((videoY ?? 0) + 200) / 400 - height / 2;
...
function CardPreview({ card, globalUrl, aspectRatio = '1:1', globalBgImage, previewWidth, videoPreviewOn, previewMuted, previewVolume, onTextClick, onCardUpdate, selectedHandle, onSelectHandle }) {
  ...
  const thumbOffX = thumbScaledW * ((card.videoX ?? 0) + 200) / 400 - previewW / 2;
  const thumbOffY = thumbScaledH * ((card.videoY ?? 0) + 200) / 400 - previewH / 2;
        ...
        React.createElement("div", { style: { flex: 1, minWidth: 0 } },
          ...
          React.createElement(Section, { title: "클립 편집" },
            ...
            React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 6, marginTop: 8 } },
              ...
              React.createElement(SliderRow, { label: "좌우", value: card.videoX ?? 0, min: -200, max: 200, step: 1, onChange: (v) => update("videoX", v), defaultValue: 0, suffix: '' }),
              React.createElement(SliderRow, { label: "위아래", value: card.videoY ?? 0, min: -200, max: 200, step: 1, onChange: (v) => update("videoY", v), defaultValue: 0, suffix: '' }),
