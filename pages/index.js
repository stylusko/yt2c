            ...
            card.appliedStart
              ...
              : React.createElement(React.Fragment, null,
                  ...
                  (() => {
                    ...
                    return React.createElement("button", {
                      onClick: () => { if (!valid) { setClipError(errors); return; } var vu = card.url || globalUrl; setVideoLoading(true); updateMulti({ appliedStart: card.start, appliedEnd: card.end, clipThumbnail: null }); if (vu && s != null) { fetch(`/api/frame?url=${encodeURIComponent(vu)}&t=${s}`).then(r => r.blob()).then(b => { var reader = new FileReader(); reader.onload = () => update("clipThumbnail", reader.result); reader.readAsDataURL(b); }).catch(() => {}); } },
