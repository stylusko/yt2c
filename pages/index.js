...
const SKIP_CARD_KEYS = new Set(['id', 'uploadedImage', 'clipThumbnail']);
const CARD_KEY_MAP = {
            ...
            card.appliedStart
              ...
              : React.createElement(React.Fragment, null,
                  ...
                  (() => {
                    ...
                    return React.createElement("button", {
                      onClick: () => { if (!valid) { setClipError(errors); return; } var vu = card.url || globalUrl; var frameUrl = vu && s != null ? `/api/frame?url=${encodeURIComponent(vu)}&t=${s}&_=${Date.now()}` : null; setVideoLoading(true); updateMulti({ appliedStart: card.start, appliedEnd: card.end, clipThumbnail: frameUrl }); if (frameUrl) { fetch(frameUrl).then(r => r.blob()).then(b => { var reader = new FileReader(); reader.onload = () => update("clipThumbnail", reader.result); reader.readAsDataURL(b); }).catch(() => {}); } },
