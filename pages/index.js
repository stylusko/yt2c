...
const DEFAULT_CARD = () => ({
  ...
  captureTime: "", videoX: 200, videoY: 200, videoScale: 100, videoBrightness: 0,
...
function SliderRow({ label, value, min, max, step, onChange, suffix = '%', defaultValue }) {
  ...
  return React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 10 } },
    ...
    React.createElement("input", { type: "range", min, max, step, value, onChange: (e) => { const v = parseFloat(e.target.value); const snap = Math.abs(max - min) * 0.02; onChange(Math.abs(v - defVal) <= snap ? defVal : v); }, style: { flex: 1, accentColor: T.accent } }),
    React.createElement("span", {
      onDoubleClick: () => onChange(defVal),
...
function VideoPreview({ videoId, start, end, width, height, videoX, videoY, videoScale, videoBrightness, muted: mutedProp, volume: volumeProp = 80 }) {
  ...
  const offX = scaledW * (videoX ?? 200) / 400 - width / 2;
  const offY = scaledH * (videoY ?? 200) / 400 - height / 2;
...
function CardPreview({ card, globalUrl, aspectRatio = '1:1', globalBgImage, previewWidth, videoPreviewOn, previewMuted, previewVolume, onTextClick, onCardUpdate, selectedHandle, onSelectHandle }) {
  ...
  const thumbOffX = thumbScaledW * (card.videoX ?? 200) / 400 - previewW / 2;
  const thumbOffY = thumbScaledH * (card.videoY ?? 200) / 400 - previewH / 2;
        ...
        React.createElement("div", { style: { flex: 1, minWidth: 0 } },
          ...
          React.createElement(Section, { title: "클립 편집" },
            ...
            React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 6, marginTop: 8 } },
              React.createElement(SectionTitleWithReset, { title: "\uD074\uB9BD \uC870\uC815", onReset: () => updateMulti({ videoX: 200, videoY: 200, videoScale: 100, videoBrightness: 0 }) }),
              React.createElement(SliderRow, { label: "좌우", value: card.videoX, min: 0, max: 400, step: 1, onChange: (v) => update("videoX", v), defaultValue: 200 }),
              React.createElement(SliderRow, { label: "위아래", value: card.videoY, min: 0, max: 400, step: 1, onChange: (v) => update("videoY", v), defaultValue: 200 }),
