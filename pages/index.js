      ...
      card.uploadedImage
        ...
        : React.createElement(React.Fragment, null,
            ...
            card.appliedStart
              ...
              : React.createElement(MobileClipSelector, { videoUrl: card.url || globalUrl, start: card.start, end: card.end, onStartChange: (v) => update("start", v), onEndChange: (v) => update("end", v), onClipChange: (s, e) => updateMulti({ start: s, end: e }), onExpandChange: (open) => { setClipSelectorOpen(open); if (onClipExpandChange) onClipExpandChange(open); }, onApply: () => { var s = parseTime(card.start), e = parseTime(card.end); if (s == null || e == null || e <= s) return; var vu = card.url || globalUrl; setVideoLoading(true); var loadTimer = setTimeout(() => setVideoLoading(false), 10000); updateMulti({ appliedStart: card.start, appliedEnd: card.end, clipThumbnail: null }); if (vu && s != null) { fetch(`/api/frame?url=${encodeURIComponent(vu)}&t=${s}`).then(r => r.blob()).then(b => { var reader = new FileReader(); reader.onload = () => { update("clipThumbnail", reader.result); clearTimeout(loadTimer); setVideoLoading(false); }; reader.readAsDataURL(b); }).catch(() => { clearTimeout(loadTimer); setVideoLoading(false); }); } else { clearTimeout(loadTimer); setVideoLoading(false); } } }),
